...
def get_frames(video_name):
    if not video_name:
        ...
        cap = cv2.VideoCapture(args.video_name)# get frames from the video
        while True:
            ret, frame = cap.read() #ret:(bool)whether successfully read the frame(false if the last frame); frame:pixel matrix of each frame
            if ret:
                yield frame
            else:
                break
    else:
        images = glob(os.path.join(video_name, '*.jp*'))#string matching, to get all the images under the path
        images = sorted(images,
                        key=lambda x: int(x.split('/')[-1].split('.')[0]))#sort according to the image name
        for img in images:
